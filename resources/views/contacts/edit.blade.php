...
@section('content')
    <div style="background: white; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);
        ...
        <h1 style="color: #2c3e50; margin-bottom: 1.5rem;">Editar Contacto</h1>
        ...
        <form action="{{ route('contacts.update', $contact) }}" method="POST">
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <textarea id="notes" name="notes" rows="3"
                    style="width: 100%; padding: 8px 12px; border: 1px solid #ccc;
                           border-radius: 6px; font-size: 1rem; resize: vertical;">{{ old('notes', $contact->notes) }}</textarea>
                @error('notes')
                    <div style="color: #e74c3c; font-size: 0.85rem; margin-top: 4px;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div style="margin-bottom: 1.5rem; display: flex; align-items: center; gap: 8px;">
                <input type="checkbox" id="favorite" name="favorite"
                    {{ old('favorite', $contact->favorite) ? 'checked' : '' }}
            ...
            <div style="display: flex; gap: 10px;">
                <button type="submit"
                    style="padding: 10px 24px; background: #f39c12; color: white;
